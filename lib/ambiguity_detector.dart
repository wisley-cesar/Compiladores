...
class AmbiguityDetector {
  ...
  void detectarAmbiguidades(String char, Function(String) adicionarErro) {
    ...
    if (char == ')' && pos + 1 < codigo.length && codigo[pos + 1] == ')') {
      adicionarErro(
        'Parêntese extra detectado - possível ambiguidade sintática',
      );
    }
    ...
    if (char == ']' && pos + 1 < codigo.length && codigo[pos + 1] == ']') {
      adicionarErro(
        'Colchete extra detectado - possível ambiguidade sintática',
      );
    }
    ...
    if (char == ';' && pos + 1 < codigo.length && codigo[pos + 1] == ';') {
      adicionarErro(
        'Ponto e vírgula duplo detectado - possível ambiguidade sintática',
      );
    }
  ...
  void _detectarPadroesProblematicos(
    String char,
    Function(String) adicionarErro,
  ) {
    // Padrão: )){ - parêntese extra seguido de chave
    if (char == ')' && pos + 2 < codigo.length) {
      if (codigo[pos + 1] == ')' && codigo[pos + 2] == '{') {
        adicionarErro(
          'Parêntese extra antes de chave - possível ambiguidade sintática',
        );
      }
    }
  ...
  void _detectarPadroesEspecificos(
    String char,
    Function(String) adicionarErro,
  ) {
    ...
    if (char == '>' && pos > 0 && pos + 1 < codigo.length) {
      ...
      if (anterior != ' ' && proximo != ' ' && proximo == ')') {
        adicionarErro(
          'Falta espaço adequado ao redor do operador - possível ambiguidade',
        );
      }
    }
    ...
    if ((char == '+' || char == '-') && pos + 2 < codigo.length) {
      ...
      if (proximo1 == char && proximo2 == char) {
        adicionarErro(
          'Operador ambíguo detectado: $char$char$char - possível ambiguidade sintática',
        );
      }
    }
  }
